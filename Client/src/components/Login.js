...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Check if user with entered email exists in Firestore
      ...
      querySnapshot.forEach(async (doc) => {
        ...
        if (userData.password === formData.password) {
          ...
          history.push('/home'); // Navigate to the dashboard page after successful login
        } else {
  ...
  return (
    <div className={styles.formContainer} style={{ background: '#252424' }}>
      <form className={styles.form} onSubmit={handleSubmit}>
        <h2 id="heading" style={{ color: 'white', display: 'flex', padding: '20px', alignContent: 'center', flexWrap: 'wrap', flexDirection: 'column-reverse' }}>Login</h2>
